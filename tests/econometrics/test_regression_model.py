...
def test_get_wls_downweights_noisy_observations():
    ...
    assert abs(result.coefficients[1] - 1.0) < abs(ols_result.coefficients[1] - 1.0)
...
def test_get_gls_identity_omega_matches_ols():
    ...
    assert np.allclose(ols_result.coefficients, gls_result.coefficients, atol=1e-6)
...
def test_get_quantile_regression_median_close_to_ols_for_symmetric_noise(recorder):
    ...
    recorder.capture(qr_result.summary().round(4))
