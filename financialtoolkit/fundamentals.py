...
def get_financial_statements(
    ...
    if statement == "balance":
        ...
        statement_format = (
            pd.read_csv(
                _BALANCE_SHEET_FORMAT_LOCATION, index_col="FinancialModelingPrep"
            )["Generic"]
            if statement_format.empty
            else statement_format
        )
    elif statement == "income":
        location = "income-statement"
        statement_format = (
            pd.read_csv(_INCOME_FORMAT_LOCATION, index_col="FinancialModelingPrep")[
                "Generic"
            ]
            if statement_format.empty
            else statement_format
        ...
        statement_format = (
            pd.read_csv(_CASH_FLOW_FORMAT_LOCATION, index_col="FinancialModelingPrep")[
                "Generic"
            ]
            if statement_format.empty
            else statement_format
    ...
    for ticker in ticker_list:
        ...
        financial_statement_dict[ticker] = convert_financial_statement(
            financial_statement, format, True
        )

    financial_statement_total = pd.concat(financial_statement_dict, axis=1)

    if len(ticker_list) == 1:
        financial_statement_total = financial_statement_total[ticker_list[0]]
